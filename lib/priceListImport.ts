...
export function parseExcelBuffer(
  ...
  for (let i = startIndex; i < rows.length; i++) {
    ...
    // Артикул сам по себе НЕ гарантирует, что строка — реальный товар:
    // в прайсах встречаются служебные строки шапки/примечаний (дата
    // выгрузки прайса, название компании, телефон, ссылка на сайт),
    // где в колонку артикула попадает произвольный текст, а цены нет
    // вовсе. Раньше строку пропускали, только если И артикул, И цена
    // были пустыми — поэтому такой "мусорный" текст с нулевой ценой
    // проходил и оседал в каталоге как настоящий товар (пример из
    // жизни: артикулы вида "СТАННАДАТУ190926", "ТЕЛ0976835234",
    // "АВТОРЫНОККУЯЛЬНИК"). Теперь достаточно отсутствия цены (<= 0),
