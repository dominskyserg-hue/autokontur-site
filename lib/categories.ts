...
export const CATEGORIES: CategoryDef[] = [
  ...
  {
    // Розділ "Ремені та ролики" (рішення власника): усе в одному місці +
    // дві підкатегорії з окремими сторінками для Google — "Ремені та
    // ролики ГРМ" і "Поліклинові ремені та ролики". Адреса лишилась стара.
    // Сюди потрапляють: товари ГРМ (правило нижче) + усі товари обох
    // підкатегорій + неоднозначні ролики/натягувачі без уточнення, до якого
    // приводу вони (lib/categoryRulesExtra.ts, lib/categoryAssignment.ts)
    slug: 'remeni-rolyky-grm',
    name: 'Ремені та ролики',
    h1: 'Ремені та ролики: ГРМ і поліклинові — купити з доставкою',
    metaTitle: 'Ремені та ролики ГРМ і поліклинові купити | DominatorParts',
    metaDescription:
      'Ремені та ролики ГРМ, поліклинові ремені генератора, кондиціонера й ГУР, натяжні та обвідні ролики для іномарок. Доставка по всій Україні.',
    intro:
      'Ремені ГРМ і поліклинові ремені навісного обладнання разом з роликами та натягувачами. Ремінь ГРМ міняйте строго за регламентом виробника — його обрив на багатьох моторах означає капітальний ремонт.',
    matchGroups: [['грм']],
    excludeWords: ['прокладк', 'шестерня', 'кришка', 'крышка'],
  },
  {
    // Підкатегорія: те, що раніше було всією категорією "Ремені та ролики ГРМ"
    slug: 'remeni-rolyky-grm-komplekty',
  ...
  {
    // Підкатегорія: поліклинові (струмкові) ремені та ролики навісного
    // обладнання — генератор, кондиціонер, ГУР, помпа
    slug: 'polyklynovi-remeni-rolyky',
    name: 'Поліклинові ремені та ролики',
    h1: 'Поліклинові ремені та ролики — купити з доставкою по Україні',
    metaTitle: 'Поліклиновий ремінь і ролики купити | DominatorParts',
    metaDescription:
      'Поліклинові ремені генератора, кондиціонера й ГУР, натяжні та обвідні ролики навісного обладнання для іномарок. Доставка по всій Україні.',
    intro:
      'Поліклиновий (струмковий) ремінь крутить генератор, кондиціонер, насос ГУР і помпу. Разом з ременем варто перевірити натяжний і обвідні ролики — їх підшипники зношуються так само.',
    matchGroups: [['поликлин', 'полікл', 'поліклін']],
    excludeWords: ['грм'],
    parentCategorySlug: 'remeni-rolyky-grm',
  },
  {
  ...
  },
  {
    ...
    // + ремені ГУР — у "Поліклинові ремені та ролики" (але не "ремінь рульової
    // рейки" — це деталь самої рейки, лишається тут)
    ...
    excludeWords: ['пильовик', 'пильник', 'сальник', 'сайлентблок', ...GUR_BELT_WORDS],
  },
  ...
  // ============================================================
  {
    ...
    // Сам ремінь генератора — не деталь генератора: він у підкатегорії
    // "Поліклинові ремені та ролики". Шків ("шків ременя") лишається тут
    excludeWords: BELT_WORDS,
  },
  {
    slug: 'prokladky-dvyhuna',
    // Назва змінена (рішення власника): додано сальники двигуна —
    // колінвалу, розподільчого валу, клапанів (маслознімні ковпачки).
    // Адреса сторінки лишилась стара, щоб Google не втратив сторінку
    name: 'Прокладки, сальники та кільця двигуна',
    h1: 'Прокладки, сальники та кільця двигуна — купити з доставкою по Україні',
    metaTitle: 'Прокладки, сальники та кільця двигуна купити | DominatorParts',
    metaDescription:
      'Прокладки ГБЦ, клапанної кришки, колектора, сальники колінвалу та розподільчого валу, маслознімні ковпачки, поршневі кільця для іномарок. Доставка по всій Україні.',
    intro:
      'Прокладки, сальники та ущільнювальні кільця двигуна — від прокладки головки блока циліндрів і сальника колінвалу до комплекту прокладок на весь двигун. У наявності деталі під різні марки й моделі.',
...
export function getNarrowVariants(parentSlug: string): CategoryDef[] {
  // Лише вузькі сторінки ПІД МОДЕЛЬ авто (з modelGroup) — вони йдуть у блок
  // "Популярні моделі". Загальні підкатегорії — getSubcategories() нижче
  return CATEGORIES.filter((c) => c.parentCategorySlug === parentSlug && Boolean(c.modelGroup));
}

// Загальні підкатегорії (НЕ під модель авто), напр. "Ремені та ролики" →
// "Ремені та ролики ГРМ" і "Поліклинові ремені та ролики". Усі їхні товари
// є і в батьківській категорії (lib/categoryAssignment.ts). На сторінці
// батьківської категорії — окремий блок "Підкатегорії"
export function getSubcategories(parentSlug: string): CategoryDef[] {
  return CATEGORIES.filter((c) => c.parentCategorySlug === parentSlug && !c.modelGroup);
}
...
// ламається
// ------------------------------------------------------------
// ПРАВИЛА КАТЕГОРІЇ → SQL-умова по назві товару
// ------------------------------------------------------------
// Використовується ЛИШЕ для пересчёта таблицы product_categories
// (lib/categoryAssignment.ts). Сторінки читають готову таблицю через
// buildCategoryWhereClause() нижче — правила при кожному відкритті
// сторінки більше не виконуються.
// Без умови is_active: у таблицю пишемо і неактивні товари, щоб при
// поверненні товару в продаж він одразу опинився в своїй категорії
// (фільтр is_active — на сторінках, у buildCategoryWhereClause)
export function buildCategoryRuleClause(
  category: CategoryDef,
...
// ------------------------------------------------------------
// ТОВАРИ КАТЕГОРІЇ — із таблиці product_categories
// ------------------------------------------------------------
// Раніше тут виконувались самі правила (ILIKE по назві) при кожному
// відкритті сторінки категорії/марки/ТО та в пошуку за авто. Тепер
// категорії товарів пораховані заздалегідь (lib/categoryAssignment.ts),
// а тут — лише перевірка "товар є в цій категорії" по індексу.
// Приховані (is_active=false) товари — тестові/сміттєві записи — як і
// раніше, в жоден список не потрапляють
export function buildCategoryWhereClause(
  category: CategoryDef,
  startParamIndex: number
): { clause: string; params: unknown[] } {
  return {
    clause: `p.is_active = true AND EXISTS (SELECT 1 FROM product_categories pc WHERE pc.product_id = p.id AND pc.category_id = $${startParamIndex})`,
