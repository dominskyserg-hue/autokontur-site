...
export interface SeoOverrideFaqItem {
  question: string;
  // Звичайний текст відповіді — АЛЕ якщо в ньому зустрічається токен
  // "{{доставка}}", lib/productDetail.ts підставить замість нього
  // РЕАЛЬНИЙ термін доставки цього конкретного товару (product.stock/
  // product.deliveryTime) на момент рендеру сторінки — тут, у
  // статичному файлі, конкретний термін хардкодити не можна: він
  // змінюється разом з наявністю на складі, а один і той же артикул
  // може бути в кількох постачальників одразу з різними термінами
  answer: string;
