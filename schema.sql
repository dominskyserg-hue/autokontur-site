...
-- ============================================================
-- 19. ТАБЛИЦА customer_discounts — персональные скидки покупателям
-- ============================================================
-- Ручное назначение постоянной скидки конкретному покупателю по
-- номеру телефона (единственный способ идентифицировать покупателя
-- на этом сайте — отдельной таблицы "клиенты"/аккаунтов с паролем
-- нет, см. components/CustomerDashboard.tsx, где вход в "Особистий
-- кабінет" тоже происходит по телефону). Скидка применяется
-- автоматически при оформлении заказа (app/api/orders/create/route.ts) —
-- покупателю не нужно вводить промокод, достаточно указать тот же
-- номер телефона, что и раньше.
--
...
CREATE TABLE IF NOT EXISTS customer_discounts (
  ...
  -- Заметка админа — для чего скидка ("постоянный клиент", "оптовик",
  -- "извинение за задержку заказа №..."). Покупателю не показывается
  note TEXT,
