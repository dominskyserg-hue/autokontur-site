...
export default function ProductGallery({ photos }: { photos: GalleryPhoto[] }) {
  ...
  return (
    <div>
      ...
      {/* Фото товарів беруться з прайс-листів постачальників "як є" —
          ніхто вручну не звіряє кожне фото з кожним артикулом (тисячі
          товарів), тому іноді на фото не той самий товар (наприклад,
          гальмівний диск замість колодок). Чесне попередження прямо
          під фото — краще для довіри покупця, ніж мовчазна помилка,
          яку він помітить, лише коли отримає посилку */}
      {active && (
        <p className="mt-2 text-center text-[11px]" style={{ color: FAINT }}>
          Фото ілюстративне — може не відповідати обраному товару
        </p>
      )}
    </div>
  );
